...
const SearchTutorialCard = ({ tutorial }) => {
  return (
    <div key={tutorial._id} className="w-full">
      <div className="bg-darkPurple-dark shadow-lg rounded-lg p-4 sm:p-6">
        ...
        <div className="font-light text-notWhite text-xs mb-2">{tutorial.platform || "Unknown Platform"}</div>
        <div className="font-light text-brightPeach text-xs mb-2">{tutorial.rating || "No Rating"}</div>
        <p className="text-white text-base mb-4">{tutorial.content}</p>
        <span className="text-white text-sm block mb-4">
          Author: {tutorial.author?.username || "Unknown Author"}
        </span>
        <Link
